...
public:
	...
	const unsigned short SENSOR_COUNT = 10;

	NNActor();
	NNActor(const std::vector<std::vector<float>> dna);
	~NNActor();
	...
	std::vector<std::vector<float>> getDna() const;
...
private:
	...
	std::vector<std::vector<float>> m_dna;
};
